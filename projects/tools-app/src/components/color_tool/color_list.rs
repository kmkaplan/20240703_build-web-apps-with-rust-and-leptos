...
#[component]
fn ColorListItem(color: Color, remove_color: Action<RemoveColor, Result<(), ServerFnError>>) 
-> impl IntoView {
    view! {
        <li>
...
pub fn ColorList(
    ...
    let color_list_view = move || -> Option<Result<View, _>> {
        color_resource.and_then(|colors: &Vec<Color>| {
            colors.iter()
            .map(|color| view! { <ColorListItem color=color.clone() remove_color=remove_color />})
            .collect_view()
    })
    };
    view! {
